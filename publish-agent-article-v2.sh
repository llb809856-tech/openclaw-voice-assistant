...
COVER_RESPONSE=$(curl -s -X POST "https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=$TOKEN&type=image" \
  -F "media=@$COVER_IMAGE" \
  -F "description={\"title\":\"AI Agent 封面\",\"introduction\":\"AI 改变工作\"}")
...
# 3. 上传文章内配图（永久素材）
echo "📌 上传文章内配图..."
INLINE_RESPONSE=$(curl -s -X POST "https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=$TOKEN&type=image" \
  -F "media=@$INLINE_IMAGE" \
  -F "description={\"title\":\"AI Agent 内图\",\"introduction\":\"AI 工作场景\"}")

INLINE_MEDIA_ID=$(echo "$INLINE_RESPONSE" | grep -o '"media_id":"[^"]*"' | cut -d'"' -f4)

if [ -z "$INLINE_MEDIA_ID" ]; then
  echo "❌ 文章内配图上传失败"
  echo "$INLINE_RESPONSE"
  exit 1
fi
echo "✓ 文章内配图上传成功：$INLINE_MEDIA_ID"

# 4. 生成带排版的 HTML（含真实图片）
echo "📌 生成排版 HTML..."

# 注意：微信图片要用 data-src 属性，不能用 src
HTML='<!DOCTYPE html>
<html>
...
<img data-src="https://mmbiz.qpic.cn/mmbiz_jpg/'$INLINE_MEDIA_ID'/640?wx_fmt=jpeg&amp;from=appmsg" style="max-width: 100%; height: auto; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);" alt="AI Agent 工作流程"/>
...
if echo "$RESULT" | grep -q '"media_id"'; then
  ...
  echo "✅ 发布成功！"
  echo "👉 标题：$TITLE"
  echo "👉 封面 Media ID: $COVER_MEDIA_ID"
  echo "👉 内图 Media ID: $INLINE_MEDIA_ID"
