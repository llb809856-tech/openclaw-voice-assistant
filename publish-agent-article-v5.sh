...
echo "🚀 微信公众号自动发布（Agent 文章 - 修复图片 v5）"
echo "================================================"
...
# 3. 上传文章内配图 + 生成 HTML（用 Python 处理）
echo "📌 上传文章内配图并生成 HTML..."
...
<body>
<section style="font-family: -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Microsoft YaHei', sans-serif; font-size: 15px; line-height: 1.6; color: #333; padding: 10px 20px;">
...
# 保存 HTML
with open('/Users/a01/.openclaw/workspace/article-content-v5.html', 'w', encoding='utf-8') as f:
    f.write(html)
...
result = response.json()
img_url = result.get('url', '').replace('http://', 'https://')
print(img_url)
PYTHON_SCRIPT
)

echo "✓ 文章内配图 URL: $IMG_URL"

# 读取 HTML 文件
HTML=$(cat /Users/a01/.openclaw/workspace/article-content-v5.html)
