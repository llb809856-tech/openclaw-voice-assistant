...
COVER_RESPONSE=$(curl -s -X POST "https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=$TOKEN&type=image" \
  -F "media=@$COVER_IMAGE" \
  -F "description={\"title\":\"AI Agent 封面\"}")
...
# 3. 上传文章内配图（使用 img 上传接口，返回 URL）
echo "📌 上传文章内配图..."

IMG_UPLOAD_RESPONSE=$(curl -s -X POST "https://api.weixin.qq.com/cgi-bin/media/uploadimg?access_token=$TOKEN" \
  -F "media=@$INLINE_IMAGE")

echo "图片上传返回：$IMG_UPLOAD_RESPONSE"

# 提取 URL 并转换为 HTTPS
IMG_URL=$(echo "$IMG_UPLOAD_RESPONSE" | grep -o '"url":"[^"]*"' | cut -d'"' -f4 | sed 's/\//g' | sed 's/^http:/https:/')

if [ -z "$IMG_URL" ]; then
  echo "❌ 文章内配图上传失败"
  echo "$IMG_UPLOAD_RESPONSE"
  exit 1
fi
echo "✓ 文章内配图上传成功，URL: $IMG_URL"

# 4. 生成带排版的 HTML（使用正确的图片 URL）
echo "📌 生成排版 HTML..."

# 使用 Python 生成 HTML，确保 URL 正确插入
...
# 输出 HTML 到文件
with open('/Users/a01/.openclaw/workspace/article-content.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("✓ HTML 已保存到：/Users/a01/.openclaw/workspace/article-content.html")
PYTHON_SCRIPT

# 读取 HTML 文件
HTML=$(cat /Users/a01/.openclaw/workspace/article-content.html)
