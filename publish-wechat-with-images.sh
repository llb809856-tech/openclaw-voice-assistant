...
# 微信公众号发布脚本（带图片和排版）
# 使用方法：./publish-wechat-with-images.sh
...
COVER_RESPONSE=$(curl -s -X POST "https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=$TOKEN&type=image" \
  -F "media=@/Users/a01/.openclaw/workspace/ai-agent-cover.jpg" \
...
# 3. 上传文章内图片（永久素材）
echo "📌 上传文章图片 1（AI 技术图）..."
IMG1_RESPONSE=$(curl -s -X POST "https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=$TOKEN&type=image" \
  -F "media=@/Users/a01/.openclaw/workspace/ai-agent-tech.jpg" \
  -F "description={\"title\":\"AI 技术图\"}")

IMG1_URL=$(echo "$IMG1_RESPONSE" | grep -o '"url":"[^"]*"' | cut -d'"' -f4)
...
<img style="max-width: 100%; height: auto; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);" src="'$IMG1_URL'" alt="AI Agent 技术">
</p>
<p style="text-align: center; font-size: 13px; color: #999; margin-top: -15px; margin-bottom: 25px;">AI Agent 技术正在重构产业</p>
...
<p style="margin: 16px 0; line-height: 1.75; text-align: center; color: #999; font-size: 13px;">本文数据来源，麦肯锡 2026 AI 应用发展核心报告、36 氪、新浪财经。</p>
