...
# 微信的 img 上传接口返回的是 URL，可以直接用在 content 里
...
# 提取 URL（微信返回的是 url 字段，不是 media_id）
IMG_URL=$(echo "$IMG_UPLOAD_RESPONSE" | grep -o '"url":"[^"]*"' | cut -d'"' -f4)
...
# 注意：微信文章图片直接用 src 属性，URL 来自 uploadimg 接口
HTML='<!DOCTYPE html>
<html>
